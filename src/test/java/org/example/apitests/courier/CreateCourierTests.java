...
@Link(url = "https://qa-scooter.praktikum-services.ru/docs/#api-Courier-CreateCourier", name = "#api-org.example.apitests.models.requestmodels.Courier-CreateCourier")
@Tag("create-courier")
...
public class CreateCourierTests extends CourierAPIHandler {
    ...
    @Description("Тест проверяет API создания нового курьера без обязательных полей. Ожидаемый результат - новый курьер НЕ создан")
    public void createCourierMissingAllParamsIsFailed() {
        Response response = createCourier("", "", firstName);
        setIsCreated(isCourierCreated(response, 201));
